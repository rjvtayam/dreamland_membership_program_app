...
export default function Sidebar() {
  ...
  return (
    <>
        ...
        <div className={cn(
          ...
          {sidebarOpen ? (
            <div className="flex items-center gap-3 w-full">
              <motion.div whileHover={{ rotate: 12, scale: 1.1 }} className="relative flex-shrink-0">
                <div className="absolute inset-0 rounded-xl blur-lg opacity-30" style={{ background: 'var(--neon)' }} />
            ...
            <>
              <motion.button
                ...
                whileTap={{ scale: 0.85 }}
                onClick={toggleSidebar}
                className="p-2 rounded-xl text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition-colors mb-1"
              >
          ...
          {navItems.map((item) => (
            <div key={item.to} className="relative">
              <NavLink
                ...
                onMouseEnter={(e) => !sidebarOpen && showTooltip(item.label, e)}
                onMouseLeave={() => !sidebarOpen && hideTooltip()}
              >
                {({ isActive }) => (
                  <>
                    <motion.div
        ...
        <div className="px-2 py-3 flex-shrink-0">
          <motion.button
            ...
            onMouseEnter={(e) => {
              ...
              if (!sidebarOpen) showTooltip('Logout', e)
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.background = 'transparent'
              if (!sidebarOpen) hideTooltip()
            }}
          >
      ...
      {/* Fixed-position tooltips — rendered outside sidebar overflow */}
      <AnimatePresence>
        {tooltip && !sidebarOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -4 }}
            transition={{ duration: 0.15 }}
            className="fixed z-[200] pointer-events-none px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap"
            style={{
              top: tooltip.top,
              left: 95,
          ...
          >
            ...
            <div className="absolute right-full top-1/2 -translate-y-1/2 w-0 h-0"
              style={{
                ...
                borderRight: '5px solid var(--surface-border)',
              }} />
          </motion.div>
        )}
      </AnimatePresence>
